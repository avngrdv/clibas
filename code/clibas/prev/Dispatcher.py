...
class Dispatcher:
    ...
    def _dispatch_Pipeline(self):
        from utils.ProcessHandlers import Pipeline
        return Pipeline(self.common)
        
    def _dispatch_FastqParser(self):

        from utils.ProcessHandlers import FastqParser
        if hasattr(self.config, 'FastqParserConfig'):
            params = self._config_to_dict(self.config.ParserConfig)
            
        if hasattr(self, 'P_design'):
            params.update(self.P_design)
    
        if hasattr(self, 'D_design'):
            params.update(self.D_design)
    ...
    def _dispatch_DataAnalysisTools(self):
        ...
        from utils.datanalysis import DataAnalysisTools
