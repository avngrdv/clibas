...
def test_array_casting_edge_cases(basic_valid_peptides):
    ...
    from clibas.datatypes import AnalysisSample, SequencingSample
    ...
    # single sequence
    single = SequencingSample(dna=np.array([b"ACGT"]), Q=np.array([b"IIII"]))
...
def test_translation_edge_cases():
    ...
    result = parser._dna_to_pep_v2(
        b"ATGTAAGGG", force_at_frame=0, stop_readthrough=False
    )
    assert result == b"M"  # stops at TAA

    # stop codon WITH readthrough
    result = parser._dna_to_pep_v2(
        b"ATGTAAGGG", force_at_frame=0, stop_readthrough=True
    )
...
def test_q_score_operations():
    ...
    sample2 = SequencingSample(
        dna=np.array([b"AC", b"ACGT"]), Q=np.array([b"II", b"IIII"])
    )
...
def test_matrix_featurization_from_F(basic_valid_peptides):
    ...
    from clibas.featurization import FeatureMatrix, from_matrix_v3

    C.initialize()

    F = FeatureMatrix.make(descr="pep_ECFP4", constants=C.analysis_tools.constants).F
